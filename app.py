...
with st.sidebar:
    ...
    with menu[1]:
        st.markdown("**指南针模式选股**")
        radar_mode = st.selectbox("核心指标", ["异动扫描", "资金净流入", "低位放量", "自然语言模式"])
        
        user_query = ""
        if radar_mode == "自然语言模式":
            user_query = st.text_area("描述你的选股逻辑", placeholder="如：回撤小且处于上涨趋势的科技股")
...
if st.session_state["radar_results"] is not None:
    with st.expander("🎯 雷达扫描结果 (点击即可研判)", expanded=True):
        for _, row in st.session_state["radar_results"].iterrows():
            c1, c2 = st.columns([5, 1])
            with c1:
                st.markdown(f"""<div style="background:#111;padding:8px 15px;border-radius:5px;border-left:4px solid #CC0000;margin-bottom:5px;">
                    <b>{row['名称']} ({row['代码']})</b> <span style="color:#FFD700;margin-left:20px;">最高涨幅: {row['最高涨幅']}</span>
                    <span style="color:#666;font-size:0.8rem;margin-left:15px;">原因: {row['理由']}</span></div>""", unsafe_allow_html=True)
            with c2:
                if st.button("审计", key=f"r_{row['代码']}", use_container_width=True):
                    st.session_state["active_ticker"] = row['代码']
                    st.session_state["report_data"] = None # 强制刷新
                    st.rerun()
...
if run_audit:
    ...
    with st.status(f"🔍 正在穿透审计: {target}...", expanded=True) as status:
        ...
        # 模拟/提取打分数据 (后续可从 CIO 裁决中通过正则提取真实分数)
        scores = [88, 72, 95, 65, 82, 90]
...
if st.session_state["report_data"]:
    ...
    c_info1.metric("标的名称", data["ctx"].get('company_name', st.session_state["active_ticker"]))
    ...
    for i, tab in enumerate(tabs):
        with tab:
            st.markdown(f'<p style="color:#888;font-size:0.8rem;">🐯 虎之眼 {data["t_names"][i]} 专家专项审计</p>', unsafe_allow_html=True)
