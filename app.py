...
with st.sidebar:
    ...
    with sb_tabs[0]:
        ...
        run_main = st.button("🚀 启动研判", type="primary", use_container_width=True)

    with sb_tabs[1]:
        strat = st.selectbox("模式", ["涨停最强", "虎之眼价值"])
        if st.button("🔭 扫描扫描", use_container_width=True):
            st.session_state["ai_results"] = engine.get_strategy_pool(strat)

    with sb_tabs[2]:
        ai_q = st.text_area("小白描述", placeholder="如：快速上涨且回撤不多")
    ...
    for _, row in st.session_state["ai_results"].iterrows():
        ...
        with c1:
            ...
            <div style="background:#111; padding:12px; border-radius:8px; border-left:4px solid #CC0000; margin-bottom:8px;">
                <b>{row['名称']} ({row['代码']})</b> <span style="color:#FFD700; margin-left:15px;">{row['胜率标签']} | 最高涨幅: {row['最高涨幅']}</span>
