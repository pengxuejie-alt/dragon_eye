...
st.markdown("""
<style>
    /* 修复长期记忆：Tab 文字精简与显示优化 */
    .stTabs [data-baseweb="tab-list"] button { font-size: 14px !important; padding: 0 12px !important; }
    .stMetric { background: #0d0d0d; border-radius: 8px; border-bottom: 3px solid #CC0000; padding: 12px; }
    .verdict-box { background: #0d0d0d; color: #FFD700; padding: 25px; border-left: 5px solid #CC0000; border-radius: 10px; min-height: 380px; line-height: 1.6;}
</style>
...
with st.sidebar:
    ...
    with menu[1]:
        ...
        if radar_mode == "自然语言模式":
            user_query = st.text_area("需求描述", placeholder="如：最近回撤小且处于上涨趋势的科技股")
        ...
        for _, row in st.session_state["radar_results"].iterrows():
            ...
            with c2:
                if st.button("审计", key=f"r_{row['代码']}", use_container_width=True):
                    ...
                    st.session_state["report_data"] = None # 清空旧数据
                    st.rerun()
...
if run_audit:
    ...
    with st.status(f"🔍 正在穿透审计: {target}...", expanded=True) as status:
        ctx = engine.get_full_context(target, target) # 使用单点穿透引擎
        if ctx["price_info"]["current_price"] == "N/A":
            st.error("⚠️ 股价读取超时，云端 IP 受限。")
            st.stop()
        ...
        # 提取 AI 分数驱动六边形图 (默认值或正则匹配)
        scores = ctx.get("auto_scores", [80, 70, 90, 60, 75, 85])
