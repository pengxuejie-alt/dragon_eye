...
class AShareDataEngine:
    ...
    def get_full_context(self, ticker_full: str, raw_ticker: str) -> dict:
        ...
        # [F1-F3] 三重行情备援
        for fn in [self._get_price_eastmoney, self._get_price_akshare, self._get_price_sina]:
            pd_ = fn(code)
            if _safe_float(pd_.get("price_info", {}).get("current_price")) is not None:
                ctx.update(pd_)
                break
        else:
            ctx.update({"price_info": {"current_price": "N/A", "change_pct": 0.0,
                                        "pe_ttm": "N/A", "pb": "N/A"},
    ...
    def _get_price_eastmoney(self, code: str) -> dict:
        ...
        try:
            ...
            params = {
                "secid":  secid,
                "fields": "f43,f44,f45,f47,f48,f57,f58,f60,f107,f116,f162,f167,f168,f169",
            ...
            mv  = _ef("f116", 1)  # 不除100，单位已是元

            # [F1] PE/PB 需额外 /100
            pe_raw = _ef("f162")   # 已除100
            pb_raw = _ef("f167")   # 已除100
            ...
            return {
                "price_info": {
                    ...
                    "change_pct":    chg,
                    "pre_close":     pcp,
                    "high_today":    _ef("f44") or "N/A",
                    "low_today":     _ef("f45") or "N/A",
                    "volume":        data.get("f47", "N/A"),
                    "amount":        data.get("f48", "N/A"),
                    "pe_ttm":        pe_raw if pe_raw and pe_raw > 0 else "N/A",
                    "pb":            pb_raw if pb_raw and pb_raw > 0 else "N/A",
                },
                "company_name": data.get("f58", code),
                "market_cap":   f"{round(mv / 1e8, 2)}亿" if mv and mv > 0 else "N/A",
                "industry":     "N/A",
            }
        except Exception:
            return _blank
    ...
    def _get_price_sina(self, code: str) -> dict:
        ...
        try:
            ...
            # [F2] 非交易时段 data[3] 可能是"0.00"，用 data[2]（昨收）兜底
            cp  = _safe_float(parts[3])
            pcp = _safe_float(parts[2])
            if not cp or cp == 0:
                cp = pcp      # 非交易时段用昨收价
            ...
            return {
                ...
                "company_name": parts[0].strip() or code,
                "market_cap":   "N/A",
                "industry":     "N/A",
            }
        except Exception:
